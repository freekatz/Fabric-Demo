...
commitChaincodeDefinition() {
  set -x
  peer lifecycle chaincode commit  -o orderer.example.com:7050 --tls true --cafile $ORDERER_CA --channelID ${CHANNEL_NAME}${CHANNEL_ID} --name ${CC_NAME} --version ${CC_VERSION} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} --peerAddresses peer0.org2.example.com:9051 --tlsRootCertFiles /opt/gopath/src/github.com/hyperledger/fabric/peer/orgs/peerOrganizations/org2.example.com/peers/peer0.org2.example.com/tls/ca.crt 
  >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
chaincodeInvokeInit() {
  ...
  peer chaincode invoke -o orderer.example.com:7050 --tls true --cafile $ORDERER_CA -C ${CHANNEL_NAME}${CHANNEL_ID} -n ${CC_NAME}  --isInit -c ${fcn_call} --peerAddresses peer0.org2.example.com:9051 --tlsRootCertFiles /opt/gopath/src/github.com/hyperledger/fabric/peer/orgs/peerOrganizations/org2.example.com/peers/peer0.org2.example.com/tls/ca.crt
  >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
chaincodeInvokeTest() {
  ## !!! 输入参数在 shell 中不要有空格
  infoln "登记病人信息"
  chaincodeInvoke '{"function":"register","Args":["h1","{\"name\":\"ZJH-1\",\"gender\":\"male\",\"birth\":\"1998-10-01\",\"identifyID\":\"ABCDEFGHIJKLMNOP\",\"phoneNumber\":\"151-2300-0000\",\"address\":\"ChongQing\",\"nativePlace\":\"NeiMengGu\",\"creditCard\":\"6217-0000-0000-0000\"}"]}' patient
  sleep 5
  infoln "查询登记结果"
  chaincodeInvoke '{"function":"query","Args":["h1"]}' patient
  sleep 5
  infoln "更新病人信息（名字和性别）"
  chaincodeInvoke '{"function":"update","Args":["h1","[\"name\",\"gender\"]","[\"ZJH-2\",\"female\"]"]}' patient
  sleep 5
  infoln "查询更新结果"
  chaincodeInvoke '{"function":"query","Args":["h1"]}' patient
  sleep 5
  infoln "删除病人信息"
  chaincodeInvoke '{"function":"delete","Args":["h1"]}' patient
  sleep 5
  infoln "查询删除结果"
  chaincodeInvoke '{"function":"query","Args":["h1"]}' patient
  sleep 5
