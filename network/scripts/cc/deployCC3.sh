...
commitChaincodeDefinition() {
  set -x
  peer lifecycle chaincode commit  -o orderer.example.com:7050 --tls true --cafile $ORDERER_CA --channelID ${CHANNEL_NAME}${CHANNEL_ID} --name ${CC_NAME} --version ${CC_VERSION} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} --peerAddresses peer0.org3.example.com:11051 --tlsRootCertFiles /opt/gopath/src/github.com/hyperledger/fabric/peer/orgs/peerOrganizations/org3.example.com/peers/peer0.org3.example.com/tls/ca.crt 
  >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
chaincodeInvokeInit() {
  ...
  peer chaincode invoke -o orderer.example.com:7050 --tls true --cafile $ORDERER_CA -C ${CHANNEL_NAME}${CHANNEL_ID} -n ${CC_NAME}  --isInit -c ${fcn_call} --peerAddresses peer0.org3.example.com:11051 --tlsRootCertFiles /opt/gopath/src/github.com/hyperledger/fabric/peer/orgs/peerOrganizations/org3.example.com/peers/peer0.org3.example.com/tls/ca.crt
  >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
